...
export function BottomTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  // Only 5 tabs visible: Home, Trips, Expense, Reminder, More
  ...
  return (
    <>
      <View style={styles.container}>
        <View style={styles.bar}>
          {visibleRoutes.map((route) => {
            ...
            const iconName = getIcon(route.name, isFocused && !showMore);
            ...
            return (
              <TabItem key={route.key} label={label} icon={iconName} focused={isFocused && !showMore} onPress={onPress} isMore={isMore} />
            );
          })}
        </View>
      </View>
      <MoreSheet
        ...
        onNavigate={(route) => {
          // @ts-ignore
          navigation.navigate(route.replace('/(app)/', '').replace('/', ''));
        }}
      />
    </>
  );
...
function getIcon(name: string, focused: boolean): keyof typeof Ionicons.glyphMap {
  const map: Record<string, keyof typeof Ionicons.glyphMap> = {
    ...
    trips: focused ? 'paper-plane' : 'paper-plane-outline',
    expense: focused ? 'wallet' : 'wallet-outline',
...
function TabItem({
  ...
  return (
    <AnimatedTouchable onPress={onPress} activeOpacity={0.7} style={[styles.item, focused && styles.itemFocused]}>
      <Animated.View style={[styles.iconWrap, focused && styles.iconWrapFocused, isMore && styles.moreWrap, animatedIcon]}>
        <Ionicons name={icon} size={isMore ? 20 : 22} color={focused ? colors.white : isMore ? colors.primary : colors.textSecondary} />
      </Animated.View>
...
const styles = StyleSheet.create({
  container: { position: 'absolute', left: 0, right: 0, bottom: 0, paddingHorizontal: 12, paddingBottom: 14 },
  bar: {
    flexDirection: 'row',
  ...
  itemFocused: {},
  iconWrap: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' },
  iconWrapFocused: { backgroundColor: colors.primary },
  moreWrap: { backgroundColor: colors.primarySurface, borderWidth: 1, borderColor: colors.primary },
