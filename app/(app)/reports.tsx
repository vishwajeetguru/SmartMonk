  ...
  return (
    <ScreenContainer safeArea padded={false} style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('screen.reports')}</Text>
        <View style={{ width: 28 }} />
      </View>
      <View style={styles.content}>
        <View style={styles.iconWrap}><Ionicons name="analytics" size={48} color={colors.primary} /></View>
        <Text style={styles.title}>{t('screen.reports')}</Text>
        <Text style={styles.subtitle}>{t('reports.subtitle')}</Text>
        <View style={styles.card}><Ionicons name="document-text-outline" size={20} color={colors.primary} /><Text style={styles.cardText}>{t('reports.comingSoon')}</Text></View>
      </View>
    </ScreenContainer>
  );
}
const makeStyles = (colors: any) => StyleSheet.create({
  ...
  iconWrap: { width: 88, height: 88, borderRadius: 44, backgroundColor: colors.primarySurface, alignItems: 'center', justifyContent: 'center' },
